...
def analyze(documents, resultsPerDocument=-1, preferNouns=False, ranking=True, files=False, verbose=False):
	...
	for doc in documents:
		...
		for (term,freq) in localWordFreqs[doc].items():
			...
			tf = float(freq*2.5)/ float(freq + 1.5*(0.25+0.75* len(doc)/avgl)) 
			idf = math.log(1 + float(len(documents) - globalWordFreq[term] + 0.5)/float(globalWordFreq[term] + 0.5) )
